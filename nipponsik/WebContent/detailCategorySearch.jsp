...
<%
   List<ProductDTO> list = (List) request.getAttribute("detailCategoryList");
	int detailCategoryCount = 0;
	if(request.getAttribute("detailCategoryCount") != null){
		detailCategoryCount = Integer.parseInt((request.getAttribute("detailCategoryCount")).toString()); 
	}  //int형 변수를 바로 변환하지 말고 valueOf 혹은 toString을 통해 변환 후 형 변환
...
<head>
<meta charset="UTF-8">
<title>NIPPONSIK <%=list.get(0).getDetail_category() %> 카테고리</title>
<script
...
<body>
	...
	<%if(list != null){ 
		String category = null;
		 if(list.get(0).getGoods_category() == 2){
			   category = "면류";
		   } else if(list.get(0).getGoods_category() == 3){
			   category = "과자";
		   } else if(list.get(0).getGoods_category() == 4){
			   category = "식재료";
		   } else if(list.get(0).getGoods_category() == 5){
			   category = "조미료";
		   } else if(list.get(0).getGoods_category() == 6){
			   category = "술";
		   } else if(list.get(0).getGoods_category() == 7){
			   category = "차";
		   } else if(list.get(0).getGoods_category() == 8){
			   category = "음료";
		   } else if(list.get(0).getGoods_category() == 9){
			   category = "가공식품";
		   } else if(list.get(0).getGoods_category() == 10){
			   category = "고기";
		   } else{
			   category = "건어물";
		   }
	...
	<section class="sectiontopnavi">
		<span><a href="index.jsp">TOP</a></span>
		<span>></span>
		<span><a href="product.nipponsik?cate_list=<%= list.get(0).getGoods_category() + 100 %>"><%= category %></a></span>
		<span>></span>
		<span><a href="detail_category.search?detail_category=<%= list.get(0).getDetail_category()%>" class="redfont"><%= list.get(0).getDetail_category() %></a></span>
	</section>
	<section class="searchoption">
		<span class="searchamount redfont">총 <%=detailCategoryCount %>건 검색</span>
		<span class="optionbox">
			<form action="select.search" method="post" id="selectForm">
			<input type="hidden" name="goods_category" value="<%=list.get(0).getGoods_category()%>">
		...
		<% for(int i= 0; i<list.size();i++){
			...
			<li>
			   <div class="innerbox">
			   <div class="imgbox">
			   <a href="product.detail?goods_code=<%=list.get(i).getGoods_code() %>" class="producttitlelink">
