...
<%
   List<ProductDTO> list = (List) request.getAttribute("list");
	List<ProductDTO> selectList = (List) request.getAttribute("selectList");
	

	int count = 0;
	if(request.getAttribute("selectListCount") != null){
		count = (Integer)request.getAttribute("selectListCount");
	}
	int selectCount = 0;
	if(request.getAttribute("selectListCount2") != null){
		selectCount = (Integer)request.getAttribute("selectListCount2");
	}

   String category = null;
   int goods_category = 0;
   if(list.size() != 0){
	   goods_category = list.get(0).getGoods_category();
   } else if(selectList.size() != 0){
	   goods_category = selectList.get(0).getGoods_category();
   }
   if(goods_category == 1){
	   category = "자사식품세트";
   } else if(goods_category == 2){
	   category = "면류";
   } else if(goods_category == 3){
	   category = "과자";
   } else if(goods_category == 4){
	   category = "식재료";
   } else if(goods_category == 5){
	   category = "조미료";
   } else if(goods_category == 6){
	   category = "술";
   } else if(goods_category == 7){
	   category = "차";
   } else if(goods_category == 8){
	   category = "음료";
   } else if(goods_category == 9){
	   category = "가공식품";
   } else if(goods_category == 10){
	   category = "고기";
   } else{
	   category = "건어물";
   }
...
<head>
<meta charset="UTF-8">
<title>NIPPONSIK <%=category %> 카테고리</title>
<script
...
<body>
	...
	<section class="sectiontopnavi">
		<span><a href="indexpage.jsp">TOP</a></span>
		<span>></span>
		<span><a href="product.nipponsik?cate_list=<%= list.get(0).getGoods_category() + 100 %>" class="redfont"><%= category %></a></span>
	</section>
	<section class="searchoption">
		<span class="searchamount redfont">총 <%=count %>건 검색</span>
		<span class="optionbox">
			<form action="select.search" method="post" id="selectForm">
			<input type="hidden" name="goods_category" value="<%=goods_category%>">
			<select name="seloption" size="1" class="seloption" id="selectOption" onchange="this.form.submit()">
		...
		<% for(int i= 0; i<list.size();i++){
			...
			<li>
			   <div class="innerbox">
			   <div class="imgbox">
			   <a href="product.detail?goods_code=<%=list.get(i).getGoods_code() %>" class="producttitlelink2">
			      <img alt="사진 없음" src="indexpicture/<%= list.get(i).getGoods_category() %>/<%=list.get(i).getGoods_code() %>.png">
			   </a>
			   </div>
			   <div class="produckdetail">
			   <div class="producttitle"><a href="product.detail?goods_code=<%=list.get(i).getGoods_code() %>" class="producttitlelink"><%=list.get(i).getGoods_name() %></a></div>
			   <div class="productcode">상품코드 : <%=list.get(i).getGoods_code() %></div>
			   <div class="productprice"><fmt:formatNumber value="<%= list.get(i).getGoods_price() %>" pattern="#,###" />원</div>
			   </div>
			   </div>
			</li>
			<%
	...
	<section class="sectiontopnavi">
		...
		<span>></span>
		<span><a href="#a" class="redfont"><%= category %></a></span>
	</section>
	<section class="searchoption">
		<span class="searchamount redfont">총 <%=selectCount %>건 검색</span>
		<span class="optionbox">
			<form action="select.search" method="post" id="selectForm">
			<input type="hidden" name="goods_category" value="<%=goods_category%>">
			<select name="seloption" size="1" class="seloption" id="selectOption" onchange="this.form.submit()">
		...
		<% for(int i= 0; i<selectList.size();i++){
			...
			<li>
			   <div class="innerbox">
			   <div class="imgbox">
			   <a href="product_<%=selectList.get(i).getGoods_code() %>.detail?goods_code=<%=selectList.get(i).getGoods_code() %>" class="producttitlelink">
			      <img alt="사진 없음" src="indexpicture/<%= selectList.get(i).getGoods_category() %>/<%=selectList.get(i).getGoods_code() %>.PNG">
			   </a>
			   </div>
			   <div class="produckdetail">
			   <div class="producttitle"><a href="product_<%=selectList.get(i).getGoods_code() %>.detail?goods_code=<%=selectList.get(i).getGoods_code() %>" class="producttitlelink"><%=list.get(i).getGoods_name() %></a></div>
			   <div class="productcode">상품코드 : <%=selectList.get(i).getGoods_code() %></div>
			   <div class="productprice"><fmt:formatNumber value="<%= selectList.get(i).getGoods_price() %>" pattern="#,###" />원</div>
			   </div>
			   </div>
			</li>
			<%
