...
<meta charset="UTF-8">
<title>MVC 게시판</title>
<link rel="stylesheet" type="text/css" href="qna_board_view.css">
<link rel="stylesheet"
...
<body>
	...
	<section class="middlesection">
		<article class="bbs-box">
			<p class="box-title">글 보기</p>
			<table>
				<tr class="bbs-mokrok">
					<td colspan="3">글 보기</td>
				</tr>
				<tr class="bbs-title-line">
					<td class="bbs-lift-line">글 제목</td>
					<td colspan="2" class="bbs-right-line"><%=article.getBOARD_SUBJECT()%></td>
				</tr>
				<tr class="bbs-title-line">
					<td class="bbs-lift-line">작성자</td>
					<td colspan="2" class="bbs-right-line"><%=article.getBOARD_cust_id()%></td>
				</tr>
				<tr class="bbs-title-line">
					<td class="bbs-lift-line">첨부된 파일</td>
					<td colspan="2" class="bbs-right-line">
						<%
				...
				<tr class="bbs-title-line">
					<td class="bbs-lift-line">내용</td>
					<td colspan="2" class="bbs-right-line"><%=article.getBOARD_CONTENT()%></td>
				</tr>
			</table>
			<div class="write-btn-box">
				<a href="boardList.bo?page=<%=nowPage%>" class="bbs-mokrok-btn">목록</a>
				<%
					if (id != null && userDTO.getCustom_id().equals(article.getBOARD_cust_id())) { // 로그인이 되어 있는 상태에서 버튼을 보이게 하기 위함
						//추후 board table내 id입력란 추가하여 id를 비교해서 보여줘야 한다!
				%>
				<a
					href="boardReplyForm.bo?board_num=<%=article.getBOARD_NUM()%>&page=<%=nowPage%>"
					class="bbs-mokrok-btn"> 답변 </a> <a
					href="boardModifyForm.bo?board_num=<%=article.getBOARD_NUM()%>"
					class="bbs-mokrok-btn"> 수정 </a> <a
					href="boardDeletePro.bo?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage%>"
					class="bbs-mokrok-btn"> 삭제 </a>
				<%
					} else if(!userDTO.getCustom_id().equals(article.getBOARD_cust_id())){
						%>
				<a
					href="boardReplyForm.bo?board_num=<%=article.getBOARD_NUM()%>&page=<%=nowPage%>"
					class="bbs-mokrok-btn"> 답변 </a> <a href="#" onclick="loginWaning()"
					class="bbs-mokrok-btn"> 수정 </a> <a href="#" onclick="loginWaning()"
					class="bbs-mokrok-btn"> 삭제 </a>

				<%					
					}
					else {%>
				<a href="#" onclick="logincheck()" class="bbs-mokrok-btn"> 답변 </a> <a
					href="#" onclick="logincheck()" class="bbs-mokrok-btn"> 수정 </a> <a
					href="#" onclick="logincheck()" class="bbs-mokrok-btn"> 삭제 </a>
				<%
					}
				%>
			</div>
